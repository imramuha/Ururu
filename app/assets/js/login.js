...
export  default class Register {
    constructor() {
        ...
        // eventlisteners for registerBtn
        if (this._registerBtn) {
            this
                ._registerBtn
                .addEventListener('click', evt => this.register(evt));
        }
        // for loginBtn
        if (this._loginBtn) {
            this
                ._loginBtn
                .addEventListener('click', evt => this.login(evt));
        }
        // for logoutBtn
        if (this._logoutBtn) {
            this
                ._logoutBtn
                .addEventListener('click', evt => this.logout(evt));
        }

        // checks if someone is logged in, based on the status the user gets more
        // privileges.
        this.loggedIn();
    };
    ...
    register() {
        ...
        // pushes the new array to the old one -> the one we retrieved earlier from our
        // local storage
    ...
    login() {
        ...
        // foreach login object, we check if our userinput matches any of the existing
        // objects
        _allUsers.forEach(function (value, i) {
            if (_usernameLogin === value.user && _passwordLogin === value.pass) {
                // console.log('correct user'); create a key loginStatus with the value of true
                ...
                // _loginButton.href = 'login.html'; if someone already logged in loop this
